...
> **进度(2026-05-12)**:Phase 1 + Phase 2 已落地推到 Steven 分支。Phase 3 的 spec PR 已写给叶洋([yeyang-v5-ask-rule-flags-schema.md](../raas/yeyang-v5-ask-rule-flags-schema.md))和陈洋([chenyang-ontology-ask-rule-fields.md](../raas/chenyang-ontology-ask-rule-fields.md))。
...
**前置(给陈洋 + 叶洋的 ask)**:
1. **陈洋**:Ontology Rule 节点加 `gating_severity` enum 字段(terminal / needs_human / flag_only)+ `required_resume_fields[]` 字段。spec → [chenyang-ontology-ask-rule-fields.md](../raas/chenyang-ontology-ask-rule-fields.md)。预计陈洋侧 3-5 天
2. **叶洋**:v5 `assembleActionObjectV4_5`:每个 step 的输出 schema 改成 `rule_flags[]` + evidence + applicable + result + next_action 风格,顶层加 `resume_augmentation`。spec → [yeyang-v5-ask-rule-flags-schema.md](../raas/yeyang-v5-ask-rule-flags-schema.md)。预计叶洋侧 1-2 周
...
| [docs/workflow-event-chain.md](../architecture/workflow-event-chain.md) | 端到端事件链。本 plan 影响"未来增量 A"那节,Phase 1 落地后更新 |
| [docs/neo4j-instance-storage-plan.md](../ontology/neo4j-instance-storage-plan.md) | RuleCheckAudit 写 Neo4j 的方案。本 plan 不冲突,正交 |
| [docs/yeyang-prompt-adapter-onboarding.md](../raas/yeyang-prompt-adapter-onboarding.md) | 给叶洋的 v4 onboarding。本 plan 的 Phase 3 是给叶洋的 v5 ask,需要单独写 v5 onboarding |
