...
4. **没有 Neo4j 实例存储** — 现在 rule-check 决策只 emit Inngest 事件,**不落到 Neo4j 做长期审计**。落实例数据的计划见 [neo4j-instance-storage-plan.md](../ontology/neo4j-instance-storage-plan.md)(走 Ontology API,不直连 Neo4j)
...
| [`docs/yeyang-prompt-adapter-onboarding.md`](../raas/yeyang-prompt-adapter-onboarding.md) | 给叶洋写的 adapter onboarding(他基于此交付了 v4) |
