...
function buildSystemPrompt(
  ...
  return [
    ...
    '  - You are ALREADY inside one step.run callback. Do NOT nest step.run and do NOT',
    '    reference `step` or `steps` inside the body. To emit an event, call',
    '    `inngest.send({ name, data })` directly (the import is already present).',
    ...
    '  - NEVER write an `import` statement inside the body — the tools you reference are',
    '    already imported at module top level. Just call them. (If you genuinely need an',
    '    extra import, declare it in the tool-call `imports` field, not in the body code.)',
