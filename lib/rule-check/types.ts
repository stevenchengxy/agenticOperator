...
export interface RuleCheckVerdict {
  ...
  audit: {
    ...
    /**
     * 安全网计数:LLM 把规则标成 FAIL 但 evidence 显示是"信息缺失"时,
     * runner.ts 自动翻成 NOT_APPLICABLE 的条数。> 0 说明本次 verdict
     * 是经过 reconcile 修正过的 —— 给 UI 提示"X 条规则因数据缺失被自动放行"。
     */
    missing_info_reclassified_count?: number;
    /**
