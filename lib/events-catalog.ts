...
export const EVENT_CATALOG: EventDef[] = [
  ...
  { name: "RESUME_DOWNLOADED", stage: "resume", kind: "domain", rate: 3102, err: 4, desc: "简历已从招聘渠道采集并下载至系统暂存区，等待自动化处理。",
    ...
    data: [["resume_file_paths", "List<String>"], ["job_requisition_id", "String"], ["job_requisition_ids", "List<String>"], ["expectation_payload", "Object"]],
    mutations: ["Job_Requisition"] },
  { name: "RESUME_PROCESSED", stage: "resume", kind: "domain", rate: 2802, err: 0, desc: "简历解析及候选人期望双写已完成，结构化数据准备进入规则校验与匹配。",
    publishers: ["ResumeParser"], subscribers: ["RuleCheck", "KG.index", "RAAS·上传状态"], wf: ["client→submit/v4.2"],
    emits: ["MATCH_RULE_CHECK_PASSED", "MATCH_RULE_CHECK_FAILED"],
    data: [["candidate_id", "String"], ["candidate_expectation", "Object"]], mutations: ["Resume", "Candidate", "Candidate_Expectation"] },
  ...
  { name: "MATCH_RULE_CHECK_PASSED", stage: "match", kind: "gate", rate: 14, err: 0, desc: "候选人简历通过 matchResume 前置规则检查（黑名单/资格门槛/竞对回流冷冻期等），进入智能匹配评分环节。",
    publishers: ["RuleCheck"], subscribers: ["Matcher"], wf: ["client→submit/v4.2"],
    emits: ["MATCH_PASSED_NEED_INTERVIEW", "MATCH_PASSED_NO_INTERVIEW", "MATCH_FAILED"],
    data: [["candidate_id", "String"], ["resume_id", "String"], ["job_requisition_id", "String"], ["client_id", "String"], ["rule_check_result", "String"], ["rule_check_reason", "String"]],
    mutations: ["Candidate_Match_Result"] },
  { name: "MATCH_RULE_CHECK_FAILED", stage: "match", kind: "gate", rate: 38, err: 0, desc: "候选人简历未通过 matchResume 前置规则检查；不进入智能匹配评分；触发审计与人工复核工作流。",
    publishers: ["RuleCheck"], subscribers: ["HitlReview"], wf: ["archive/v1"],
    emits: [],
    data: [["candidate_id", "String"], ["resume_id", "String"], ["job_requisition_id", "String"], ["client_id", "String"], ["rule_check_result", "String"], ["rule_check_reason", "String"], ["failed_rules", "List<JSON>"]],
    mutations: ["Candidate_Match_Result"] },
