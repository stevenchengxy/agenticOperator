...
| must-fail rules | 10-49 | 10-49:bytedance_former_employee, 10-5:hard_requirement_mismatch |
...
- ✅ **must-not-fail rule: 10-26** — applicable=true result=PASS
...
|---|---|---|---|---|---|
| 10-10 | 平均每段工作时长约3年（2018-07至2021-02, 2021-03至2024-08），稳定性良好，无消极空窗理由。 | 平均每段工作时长约, 稳定性良好, 无消极空窗理由 | 2018-07, 2021-02 | 平均每段工作时长约, 稳定性良好 | ✓ |
...
| 10-5 | 岗位要求城市为上海，候选人目前在杭州，且未明确表示接受异地入职；简历中未提及海外业务经验（加分项未命中不影响此条，但城市偏差标记风险）。 | 岗位要求城市为上海, 候选人目前在杭州, 且未明确表示接受异地入职 | — | 岗位要求城市为上海, 候选人目前在杭州 | ✓ |
...
| 10-8 | 外包接受度为“接受”，劳务形式偏好为“正编”，与岗位招聘类型“正编”匹配。 | 外包接受度为, 接受, 劳务形式偏好为 | 接受, 正编 | 外包接受度为, 劳务形式偏好为 | ✓ |
| 10-9 | 2021-02至2021-03空窗1个月，小于3个月阈值。 | 空窗, 个月, 小于 | 2021-02, 2021-03 | 空窗, 个月 | ✓ |
...
| 10-34 | 候选人曾就职于字节跳动，但未显示是通过友商派驻（简历显示为直接经历），不触发冷冻期拦截。 | 候选人曾就职于字节跳动, 但未显示是通过友商派驻, 简历显示为直接经历 | — | 候选人曾就职于字节跳动, 但未显示是通过友商派驻 | ✓ |
| 10-49 | 候选人 2018-07 至 2021-02 在字节跳动任职前端工程师，属于正编回流，需挂起流程校验凭证。 | 候选人, 在字节跳动任职前端工程师, 属于正编回流 | 2018-07, 2021-02 | 候选人, 在字节跳动任职前端工程师 | ✓ |

## 4. LLM 原始输出(full JSON)

```json
{
  "candidate_id": "C_24095d5c",
  ...
  "rule_flags": [
    {
      ...
      "applicable": true,
      "result": "PASS",
      "evidence": "平均每段工作时长约3年（2018-07至2021-02, 2021-03至2024-08），稳定性良好，无消极空窗理由。",
    ...
    },
    {
      ...
      "applicable": true,
      "result": "FAIL",
      "evidence": "岗位要求城市为上海，候选人目前在杭州，且未明确表示接受异地入职；简历中未提及海外业务经验（加分项未命中不影响此条，但城市偏差标记风险）。",
      "next_action": "block"
    },
    {
      "rule_id": "10-54",
    ...
    },
    {
      ...
      "applicable": true,
      "result": "PASS",
      "evidence": "外包接受度为“接受”，劳务形式偏好为“正编”，与岗位招聘类型“正编”匹配。",
      "next_action": "continue"
    },
    {
      "rule_id": "10-9",
      ...
      "applicable": true,
      "result": "PASS",
      "evidence": "2021-02至2021-03空窗1个月，小于3个月阈值。",
      "next_action": "continue"
    },
    {
      "rule_id": "10-1",
    ...
    },
    {
      ...
      "applicable": true,
      "result": "PASS",
      "evidence": "候选人曾就职于字节跳动，但未显示是通过友商派驻（简历显示为直接经历），不触发冷冻期拦截。",
      "next_action": "continue"
    },
    {
      "rule_id": "10-36",
    ...
    },
    {
      ...
      "applicable": true,
      "result": "FAIL",
      "evidence": "候选人 2018-07 至 2021-02 在字节跳动任职前端工程师，属于正编回流，需挂起流程校验凭证。",
...
- **RuleCheckAudit** `rca_run_2026-05-12T04-54-42-449Z_081848_s06-bytedance-history-pause`
  - run_id: `run_2026-05-12T04-54-42-449Z_081848`
  - decision: FAIL / FAIL
  - dims: client=`字节` BG=`TikTok`
  - LLM: model=`google/gemini-3-flash-preview` duration=18979 ms tokens=10128/3490
...
**trace_id**: `trace_Z_081848_s06-by_bead79` — 用这个串联 RAAS / AO / LLM / Neo4j 所有 hop

| Δt | hop | message |
|---|---|---|
| +0ms | `event-emit` | [raas-mock] emit RESUME_DOWNLOADED envelope candidate=c01-zhangsan-clean jd=jr-bytedance-tiktok-fe |
| +0ms | `raas-api-call` | POST /api/v1/candidates upload=upl_s06-bytedance-history-pause_73e0cb |
| +1ms | `raas-api-resp` | candidate_id=C_24095d5c resume_id=R_fd1c10d0 |
| +1ms | `event-emit` | [ao] emit RESUME_PROCESSED candidate=C_24095d5c jr=jr_w66 |
...
| +18984ms | `llm-response` | model=google/gemini-3-flash-preview latency=18979ms tokens=10128/3490 |
| +18984ms | `verdict` | decision=FAIL llm_decision=FAIL rules_evaluated=28/51 failures=10-49:bytedance_former_employee,10-5:hard_requirement_mismatch |
| +19008ms | `neo4j-write` | wrote RuleCheckAudit rca_run_2026-05-12T04-54-42-449Z_081848_s06-bytedance-history-pause + 16 flags + :Candidate / :Resume / :JR anchors |
| +19008ms | `event-emit` | [ao] emit RULE_CHECK_FAILED reasons=10-49:bytedance_former_employee,10-5:hard_requirement_mismatch |
