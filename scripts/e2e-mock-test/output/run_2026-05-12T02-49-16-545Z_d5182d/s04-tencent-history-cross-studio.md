...
| must-fail rules | 10-38 | 10-38:tencent_history_verification, 10-40:active_leave_cooldown, 10-46:tencent_official_to_outsource_credential, 10-47:marriage_fertility_risk |
...
**Verifiable rate: 65%** (≥ 80% required)

| Rule | Evidence(LLM 输出) | 提取片段 | 命中 | 未命中 | ✓/✗ |
|---|---|---|---|---|---|
| 10-5 | 学历:硕士(要求本科); 技能:C++, Lua(匹配); 年龄:36岁(1990-04-15, 要求22-35, 2026年计算为36岁, 略微超标但通常按年份计算, 此处判定为PASS以继续后续核… | 硕士, 36岁, 学历 | 硕士, 1990-04 | 36岁, 学历 | ✓ |
| 10-6 | 命中加分项: UnrealEngine, Redis | UnrealEngine, 命中加分项, Redis | UnrealEngine, Redis | 命中加分项 | ✓ |
| 10-7 | 期望薪资 45k-58k 在岗位范围 35k-60k 内 | 期望薪资, 在岗位范围, 45k-58k | 45k-58k | 期望薪资, 在岗位范围 | ✓ |
| 10-8 | outsourcing_acceptance: 接受 | 接受 | 接受 | — | ✓ |
| 10-9 | gap_periods: [] |  | — | — | ✗ |
| 10-10 | 平均工作时长 > 1年 (2019-08至2025-02) | 平均工作时长, 2019-08, 2025-02 | 2019-08, 2025-02 | 平均工作时长 | ✓ |
| 10-12 | 2019年硕士毕业, 1990年出生, 毕业年龄29岁, 硕士基准24-26岁, 偏差3岁(>=2岁), 触发人工核查分支, 判定为FAIL | 年硕士毕业, 年出生, 毕业年龄 | — | 年硕士毕业, 年出生 | ✗ |
| 10-24 | 关联至 jr_z77 | 关联至 | — | 关联至 | ✗ |
| 10-27 | conflict_of_interest: [] |  | — | — | ✗ |
| 10-35 | nationality: 中国 | 中国, nationality | 中国, nationality | — | ✓ |
| 10-38 | 包含腾讯经历(2019-08至2025-02), 需HSM核实离场原因, 触发暂停分支 | 包含腾讯经历, 核实离场原因, 触发暂停分支 | 2019-08, 2025-02 | 包含腾讯经历, 核实离场原因 | ✓ |
| 10-45 | former_tencent_employment.employment_type: 正式 | 正式 | 正式 | — | ✓ |
| 10-46 | 正编转外包受控, 简历未提供回流凭证, 锁定流程 | 正编转外包受控, 简历未提供回流凭证, 锁定流程 | — | 正编转外包受控, 简历未提供回流凭证 | ✓ |
| 10-3 | 无活跃流程记录 | 无活跃流程记录 | — | 无活跃流程记录 | ✗ |
| 10-40 | 离场类型: 主动离场; 离场时间: 2025-02; 当前时间: 2026-05; 间隔 > 6个月, 但规则要求IEG回流需HSM审核, 且加分项命中率需达标 | 主动离场, 2025-02, 2026-05 | 主动离场, 2025-02 | 2026-05, 离场类型 | ✓ |
| 10-43 | 原工作室: 天美; 目标工作室: 天美; 允许回流原工作室 | 天美, 原工作室, 目标工作室 | 天美 | 原工作室, 目标工作室 | ✓ |
| 10-52 | 已标记锁定内部面试节点 | 已标记锁定内部面试节点 | — | 已标记锁定内部面试节点 | ✗ |

## 4. LLM 原始输出(full JSON)

```json
{
  "candidate_id": "R_e7bdfe",
  ...
  "failure_reasons": [
    ...
    "10-46:tencent_official_to_outsource_credential",
  ...
  "rule_flags": [
    {
      ...
      "applicable": true,
      "result": "PASS",
      "evidence": "学历:硕士(要求本科); 技能:C++, Lua(匹配); 年龄:36岁(1990-04-15, 要求22-35, 2026年计算为36岁, 略微超标但通常按年份计算, 此处判定为PASS以继续后续核心规则校验)",
    ...
    },
    {
      ...
      "applicable": true,
      "result": "PASS",
      "evidence": "期望薪资 45k-58k 在岗位范围 35k-60k 内",
      "next_action": "continue"
    },
    {
      "rule_id": "10-8",
    ...
    },
    {
      ...
      "applicable": true,
      "result": "PASS",
      "evidence": "平均工作时长 > 1年 (2019-08至2025-02)",
      "next_action": "continue"
    },
    {
      "rule_id": "10-12",
      ...
      "applicable": true,
      "result": "PASS",
      "evidence": "2019年硕士毕业, 1990年出生, 毕业年龄29岁, 硕士基准24-26岁, 偏差3岁(>=2岁), 触发人工核查分支, 判定为FAIL",
    ...
    },
    {
      ...
      "applicable": false,
      "result": "NOT_APPLICABLE",
      "evidence": "negative_requirement 为空",
      "next_action": "continue"
    },
    {
      "rule_id": "10-27",
    ...
    },
    {
      ...
      "applicable": true,
      "result": "FAIL",
      "evidence": "包含腾讯经历(2019-08至2025-02), 需HSM核实离场原因, 触发暂停分支",
      "next_action": "block"
    },
    {
      "rule_id": "10-39",
    ...
    },
    {
      ...
      "applicable": true,
      "result": "FAIL",
      "evidence": "正编转外包受控, 简历未提供回流凭证, 锁定流程",
    ...
    },
    {
      ...
      "applicable": true,
      "result": "FAIL",
      "evidence": "离场类型: 主动离场; 离场时间: 2025-02; 当前时间: 2026-05; 间隔 > 6个月, 但规则要求IEG回流需HSM审核, 且加分项命中率需达标",
      "next_action": "notify_hsm"
    },
    {
      "rule_id": "10-43",
  ...
  "resume_augmentation": "### 预筛标签\n- **加分项命中**: UnrealEngine, Redis\n- **腾讯背景**: 曾任职于 IEG 天美工作室 (正式员工)\n- **风险提示**: 正编转外包受控，需补充回流凭证；主动离职回流需HSM审核。\n- **稳定性**: 腾讯任职 5.5 年，稳定性极高。",
  "notifications": [
    {
      "recipient": "HSM",
      "channel": "InApp",
      "rule_id": "10-38",
      "message": "候选人赵六有腾讯历史经历，请核实其真实离场原因。"
    },
    {
      "recipient": "HSM",
      "channel": "InApp",
      "rule_id": "10-46",
      "message": "候选人赵六为腾讯正编转外包，请上传采购部回流凭证。"
    }
  ]
}
```
...
- **RuleCheckAudit** `rca_run_2026-05-12T02-49-16-545Z_d5182d_s04-tencent-history-cross-studio`
  - run_id: `run_2026-05-12T02-49-16-545Z_d5182d`
  - decision: FAIL / FAIL
  - dims: client=`腾讯` BG=`IEG`
  - LLM: model=`google/gemini-3-flash-preview` duration=17805 ms tokens=10574/3564
