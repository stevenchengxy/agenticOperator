// POST /api/manage/runs/[id]/replay
//
// Re-publishes the run's original trigger event, creating a new execution
// from the start. This is the spec-preferred form over /restart.
//
...
export async function POST(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: `Run status '${run.status}' cannot be replayed. Expected: failed, completed, or timed_out.`,
        },
        { status: 409 },
      );
    }
    ...
    console.error('[manage/runs/replay] failed:', (e as Error).message);
