...
describe('POST /api/manage/runs/[id]/cancel', () => {
  ...
  it('returns 409 when run status is not cancellable', async () => {
  ...
  it('happy path: updates status to interrupted and writes AuditLog', async () => {
    (prisma.workflowRun.findUnique as any).mockResolvedValue({ id: 'r1', status: 'running' });
    (prisma.workflowRun.update as any).mockResolvedValue({ id: 'r1', status: 'interrupted' });
    (prisma.humanTask.updateMany as any).mockResolvedValue({ count: 0 });
    (prisma.auditLog.create as any).mockResolvedValue({});

    const res = await POST(
      new Request('http://x', { method: 'POST', body: JSON.stringify({ reason: 'stuck' }) }),
      ctx('r1'),
    );
