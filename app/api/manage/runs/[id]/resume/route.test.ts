...
describe('POST /api/manage/runs/[id]/resume', () => {
  ...
  it('returns 409 when run is not paused', async () => {
    ...
    const j = await res.json();
    expect(j.error).toBe('NOT_PAUSED');
  });

  it('happy path: updates status to running and writes AuditLog', async () => {
    (prisma.workflowRun.findUnique as any).mockResolvedValue({ id: 'r1', status: 'paused' });
    (prisma.workflowRun.update as any).mockResolvedValue({ id: 'r1', status: 'running' });
    (prisma.auditLog.create as any).mockResolvedValue({});

    const res = await POST(
      new Request('http://x', { method: 'POST', body: JSON.stringify({ reason: 'hold lifted' }) }),
      ctx('r1'),
    );
    ...
    expect(prisma.workflowRun.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'r1' },
        data: expect.objectContaining({ status: 'running', suspendedReason: null }),
      }),
    );
    expect(prisma.auditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          eventName: 'manage.run.resume',
          traceId: 'r1',
        }),
      }),
    );
